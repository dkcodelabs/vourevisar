...
BEGIN
    ...
    -- SLO 4: Recorrência (simulada por enquanto, idealmente precisa de histórico de reabertura)
    -- Vamos usar occurrence_count > 5 em erros resolvidos como proxy ou criar tabela de transição de status
    -- Por enquanto, vamos contar erros com status 'resolved' que foram atualizados recentemente mas criados muito antes? 
    -- Não, melhor usar a definição direta: Erros reabertos. 
    -- Como não temos histórico de status change, vamos usar uma aproximação:
    -- Erros 'resolved' com occurrence_count > 10 (indicando que continuaram acontecendo) ou 
    -- simplesmente retornar NULL se não tivermos dados precisos.
    -- Vamos usar: Erros marcados como resolved mas que tiveram updated_at > resolved_at?
    -- Assumindo que o sistema atualiza updated_at no "dedupe".
...
-- Função para verificar alertas (Sera chamada via RPC ou Cron externo, aqui deixamos pronta)
...
BEGIN
    ...
    -- ALERTA C: módulo core com aumento > 200% em 1h vs média 24h (Simplificado para > 10 erros em 1h se a média for baixa)
    -- Para evitar complexidade excessiva, vamos disparar se um módulo core tiver > 20 erros na última hora.
    -- (O cálculo de média móvel real exigiria mais infraestrutura de dados).
    INSERT INTO admin_alert_events (alert_type, message, metadata, status)
    SELECT 
        'module_explosion', 
