...
DROP FUNCTION IF EXISTS log_admin_error(uuid, text, text, text, text, text, text, boolean, uuid, jsonb);
...
-- Recreate with robust deduplication logic (UPDATE else INSERT)
...
BEGIN
    ...
    IF p_fingerprint IS NOT NULL THEN
        ...
        IF v_existing_id IS NOT NULL THEN
            ...
            SET 
                ...
                updated_at = NOW(),
                last_seen_at = NOW(),
                -- Update metadata to latest (merge or replace? replace for now to keep size check)
                metadata = p_metadata
    ...
    -- 2. Insert new error if not found or no fingerprint
    ...
    -- 3. Trigger Alert Check (Async-ish usually, but here sync is fine as it's fast)
    -- Or leave it to the frontend/service to call? 
    -- The service calls `check_error_alerts` separately, so we don't strictly need it here.
    -- But strict separation of concerns suggested Trigger in DB. 
    -- Let's NOT call it here to avoid recursion or complex permission issues, keeping it simple.
