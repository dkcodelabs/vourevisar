-- Migration: Quick Fix para Setup Administrativo
-- Data: 2024-12-10
-- Objetivo: Resolver problemas de RLS e criar funções essenciais

-- 1. Criar tabela user_roles com RLS mais permissivo
...
-- 4. Criar políticas mais permissivas para setup inicial
CREATE POLICY "Allow authenticated users to read roles" ON user_roles
  FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY "Allow authenticated users to insert first owner" ON user_roles
...
-- 5. Criar tabela user_subscriptions
...
CREATE POLICY "Allow users to view own subscriptions" ON user_subscriptions
  FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Allow users to manage own subscriptions" ON user_subscriptions
  FOR ALL USING (auth.uid() = user_id);

CREATE POLICY "Allow admins to manage all subscriptions" ON user_subscriptions
  FOR ALL USING (
    EXISTS(SELECT 1 FROM user_roles WHERE user_id = auth.uid() AND role IN ('admin', 'owner'))
  );

-- 7. Função get_subscription_info (versão simples)
