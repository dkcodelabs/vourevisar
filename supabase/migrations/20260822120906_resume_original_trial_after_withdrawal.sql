-- A paid subscription temporarily supersedes an existing free trial. If the
-- paid contract is withdrawn and refunded while the original trial is still
-- valid, restore only the remaining original entitlement; never create or
-- extend a second trial.
