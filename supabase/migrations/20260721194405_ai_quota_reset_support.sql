...
COMMENT ON COLUMN public.user_subscriptions.ai_quota_reset_at IS
  'Timestamp from which AI usage is counted after an authorized courtesy reset.';
...
DECLARE
  ...
  v_ai_quota_reset_at timestamptz;
...
BEGIN
  ...
  INTO v_plan, v_status, v_trial_ends_at, v_subscription_ends_at,
       v_next_billing_date, v_ai_quota_reset_at
  ...
  IF v_usage_period = 'monthly' THEN
    ...
    WHERE user_id = p_user_id
      ...
      AND created_at >= GREATEST(
        ...
        COALESCE(v_ai_quota_reset_at, '-infinity'::timestamptz)
      );
  ELSE
    SELECT COUNT(*)::integer INTO v_usage
    FROM public.user_editais
    WHERE user_id = p_user_id
      AND ai_extraction_used = true
      AND created_at >= COALESCE(v_ai_quota_reset_at, '-infinity'::timestamptz);
  END IF;
