-- The preceding replacement still referenced the retired incident log.
-- This is the authoritative purge implementation for the current schema.
...
BEGIN
  ...
  LIMIT 1;
  IF v_caller_role IS NULL THEN RAISE EXCEPTION 'Permissão negada.'; END IF;

  SELECT email INTO v_target_email FROM auth.users WHERE id = p_target_user_id;
  IF v_target_email IN ('vourevisar@gmail.com', 'darcili@gmail.com') THEN RAISE EXCEPTION 'Usuário protegido.'; END IF;
  IF p_target_user_id = auth.uid() THEN RAISE EXCEPTION 'Auto-exclusão não permitida.'; END IF;

  DELETE FROM public.billing_refund_admin_actions
  WHERE billing_refund_request_id IN (SELECT id FROM public.billing_refund_requests WHERE user_id = p_target_user_id)
  ...
  DELETE FROM public.practice_items WHERE package_id IN (SELECT id FROM public.practice_packages WHERE user_id = p_target_user_id);
  ...
  UPDATE public.comments SET parent_id = NULL
  ...
  UPDATE public.user_feedback_events SET assigned_to = NULL WHERE assigned_to = p_target_user_id;
  UPDATE public.admin_alert_events SET acknowledged_by = NULL WHERE acknowledged_by = p_target_user_id;
  UPDATE public.public_editais SET created_by = NULL WHERE created_by = p_target_user_id;
  UPDATE public.system_settings SET updated_by = NULL WHERE updated_by = p_target_user_id;
  UPDATE public.user_roles SET assigned_by = NULL WHERE assigned_by = p_target_user_id;
  ...
  VALUES (auth.uid(), 'admin_purge_user', 'auth.users', jsonb_build_object(
    'purged_user_id', p_target_user_id, 'purged_email', v_target_email, 'purged_at', now()
  ));
  DELETE FROM public.audit_logs WHERE user_id = p_target_user_id;
