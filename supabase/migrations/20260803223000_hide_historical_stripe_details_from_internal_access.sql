-- An access grant may be the current entitlement while an older Stripe
-- subscription remains stored for audit. Do not expose that historical
-- financial record as the user's current plan or payment method.
