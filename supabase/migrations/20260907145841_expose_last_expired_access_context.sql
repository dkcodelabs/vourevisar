-- Keep the current entitlement contract intact while exposing the last
-- naturally-ended access period to the signed-in owner. This powers factual
-- recovery copy without leaking the free-form administrative reason.
...
DECLARE
  ...
  expired_grant_record public.billing_access_grants%ROWTYPE;
  ...
  expired_subscription_end timestamptz;
  last_expired_access jsonb := NULL;
...
BEGIN
  ...
  SELECT access_grant.*
  INTO expired_grant_record
  FROM public.billing_access_grants AS access_grant
  WHERE access_grant.user_id = caller_id
    AND access_grant.revoked_at IS NULL
    AND access_grant.ends_at <= now()
    AND access_grant.source IN ('trial', 'manual', 'goodwill')
  ORDER BY access_grant.ends_at DESC
  LIMIT 1;
  ...
  IF NOT is_active THEN
    expired_subscription_end := CASE
      WHEN withdrawal_canceled THEN refund_record.requested_at
      WHEN subscription_record.status IN ('canceled', 'incomplete_expired') THEN subscription_effective_end
      WHEN subscription_effective_end <= now() THEN subscription_effective_end
      ELSE NULL
    END;

    IF expired_subscription_end IS NOT NULL
      AND (
        expired_grant_record.id IS NULL
        OR expired_subscription_end >= expired_grant_record.ends_at
      )
    THEN
      last_expired_access := jsonb_build_object(
        'kind', 'paid_subscription',
        'ended_at', expired_subscription_end
      );
    ELSIF expired_grant_record.id IS NOT NULL THEN
      last_expired_access := jsonb_build_object(
        'kind', CASE
          WHEN expired_grant_record.source = 'trial' THEN 'initial_trial'
          ELSE 'courtesy'
        END,
        'ended_at', expired_grant_record.ends_at
      );
    END IF;
  END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'last_expired_access', last_expired_access,
