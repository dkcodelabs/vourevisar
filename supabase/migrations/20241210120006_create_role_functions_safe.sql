-- Migration: Criar funções RPC para gerenciamento de roles (SAFE VERSION)
-- Data: 2024-12-10
...
BEGIN
  ...
  -- Política para owners gerenciarem todas as roles
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'user_roles' 
    AND policyname = 'Owners can manage all roles'
  ) THEN
...
-- 6. Função para verificar se é owner
...
-- 7. Função para verificar se é admin (ou superior)
...
-- 8. Função para obter a role mais alta de um usuário
...
-- 9. Função para atribuir role (apenas owners)
...
-- 10. Função para remover role (apenas owners)
...
-- 11. Função para obter todas as roles (admin only)
