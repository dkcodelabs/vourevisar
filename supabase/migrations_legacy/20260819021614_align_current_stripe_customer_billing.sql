-- The Stripe account/customer mapping is the boundary of the current billing
-- universe. A user can have old subscriptions left behind after a Stripe
-- account rotation; those rows remain audit data but must not drive access,
-- plan, amount, card, or cancellation state.
