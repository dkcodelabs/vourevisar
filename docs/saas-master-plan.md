...
- Medição semanal do Treino em 2026-09-05: conclusão **investigar**, sem mudança de código. Na janela atual de 7 dias houve 42 sessões, 26 com tentativa (61,9%), 16 concluídas (38,1%), recorrência de 100% entre o usuário observado e 45 tentativas; as 3 gerações foram bem-sucedidas, sem geração travada, 30 itens aceitos, 6 avaliações positivas, nenhuma denúncia e custo médio estimado de US$ 0,026496 por geração concluída. A janela anterior teve 6 falhas em 12 gerações (50%), e o trailing de 28 dias acumula 6 falhas em 15 gerações (40%); investigar a origem desse lote histórico antes de qualquer otimização. A amostra de funil permanece insuficiente para decisões de produto: 3 usuários e 32 sessões com tentativa no trailing de 28 dias.
- Nova leitura em 2026-09-05 às 15:08 BRT: conclusão **investigar**, sem ação corretiva nova. A janela atual passou a 39 sessões, 24 com tentativa (61,5%), 16 concluídas (41,0%) e 43 tentativas; as 2 gerações atuais foram bem-sucedidas, sem jobs travados, com custo médio estimado de US$ 0,032998. O lote anterior continua com 6 falhas em 13 gerações (46,2%) e o trailing de 28 dias permanece em 6 falhas em 15 gerações (40%); não surgiram denúncias nem alerta operacional atual. A amostra continua insuficiente para otimização do funil (3 usuários e 32 sessões com tentativa no trailing).
- Medição de 2026-09-06 às 00:39 BRT: conclusão **investigar**, sem ação corretiva nova. A janela atual registrou 31 sessões, 20 com tentativa (64,5%), 14 concluídas (45,2%), 34 tentativas e recorrência de 100% no único usuário observado; as 2 gerações foram bem-sucedidas, sem jobs travados, 20 itens aceitos, 5 avaliações positivas e custo médio estimado de US$ 0,032998. A janela anterior ainda concentra 6 falhas em 13 gerações (46,2%), enquanto o trailing de 28 dias permanece em 6 falhas em 15 gerações (40%); não houve nova falha, denúncia ou custo anormal na janela atual. O funil continua abaixo do limiar decisório, com 3 usuários e 32 sessões com tentativa no trailing.
- Medição de 2026-09-06 às 00:34 BRT: conclusão **investigar**, sem ação corretiva nova. Os indicadores permaneceram iguais à leitura anterior: 31 sessões, 20 com tentativa (64,5%), 14 concluídas (45,2%), 34 tentativas; 2 gerações bem-sucedidas, nenhum job travado, 20 itens aceitos, 5 avaliações positivas e custo médio estimado de US$ 0,032998. A janela anterior mantém 6 falhas em 13 gerações (46,2%) e o trailing de 28 dias 6 falhas em 15 gerações (40%); sem novo alerta operacional, de qualidade ou custo. Funil ainda insuficiente, com 3 usuários e 32 sessões com tentativa no trailing.
...
- Comparação entre giros implementada em 2026-09-04: migration `20260905021553_add_atomic_cycle_rotation_snapshots.sql`, dispatcher `user-rpc`, serviço/hook independente e card responsivo na Evolução. A transação foi ensaiada no banco com rollback e comprovou um único snapshot/incremento, retry idempotente, ownership e bloqueio de escrita direta. Estados completo, escopo alterado, insuficiente, loading e erro possuem cobertura automatizada; a tela foi validada autenticada em desktop e 375 px.
- [x] Unificar os carregamentos das rotas: as áreas de aluno aguardam a confirmação de acesso antes de montar o `AppLayout`; a marca não é mais usada como spinner; espera de página usa indicador neutro e opaco, e cada consulta de tela usa um estado local sem texto concorrente. O rótulo transitório de assinatura no rodapé da navegação também passou a usar placeholder discreto. A validação autenticada conferiu Painel, Evolução, Revisões, Ciclo de Estudos, Editais e Treino sem logo de carregamento ou textos simultâneos.
- Marca premium v4 preparada para publicação em 2026-09-04: as exportações foram reproduzidas a partir de `brand-geometry.json`, SVGs e dimensões dos PNGs/ICO foram validados, e a cópia duplicada `docs/docs/brand-kit-premium` foi retirada do workspace sem perder o kit canônico. O asset público dos e-mails passou de 2048×1792 para 1024×1024 e o SHA-256 remoto coincide com `public/brand/vourevisar-mark-dark-bg.png` (`a41da312856cefcd4a16d68fb3e17f499f7169bae1ec19b6adfbb280a1a284c2`). O relatório agregado do Treino foi reexecutado em produção somente leitura e manteve três janelas, denominadores explícitos e saúde operacional sem gerações travadas.
...
- [x] Seleção assistida de matérias no carregamento do ciclo, preservando o padrão simples de carregar todas. Entregue em 2026-09-07: todas começam selecionadas, a recomendação explica por que manter o edital completo e a lista de checkboxes só abre sob demanda; a carga atômica usa exclusivamente as matérias confirmadas. O fluxo autenticado real foi validado no edital de Catálogo da Polícia Federal (9 matérias/309 tópicos): o modal exibiu `9 de 9 matérias no ciclo` e `Escolher matérias` antes de Mesclar/Substituir, sem persistir alteração. Testes de contrato e interface, typecheck, lint, gate arquitetural, suíte completa e build passaram.
