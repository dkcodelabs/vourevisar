...
- [ ] **Homologação financeira controlada restante.** Os ensaios anteriores já comprovam mensal/anual e `invoice.payment_failed` no Stripe Test; o código mantém tentativa/ledger idempotentes e o teste de segurança passou em 2026-08-31. Restam a execução controlada de clique/retry, entrega duplicada/fora de ordem e reembolso `pending`/`failed`. O projeto isolado `vourevisar-billing-test` não pode receber o schema atual: sua história de migrations é de outra linha do produto e o `db push --dry-run` recusou aplicar mudanças em 2026-08-31. Criar ou resetar um projeto Supabase de Teste com a linha atual antes de disparar esses eventos; não usar nem alternar secrets Live da produção.
