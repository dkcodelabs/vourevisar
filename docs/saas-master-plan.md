...
- [ ] Tratar revisões manuais após a quarta revisão como contato distinto, sem corromper o programa adaptativo já concluído. Em 2026-09-01, os limites do motor (primeiro contato, R1–R4, encerramento após o quarto intervalo) e os contratos de agenda passaram nos testes focados; a sequência completa de cinco contatos agora também está coberta por teste determinístico com estado persistível entre etapas. Em 2026-09-01, uma 1ª revisão foi registrada na conta de ensaio pelo fluxo real; a agenda persistida mudou de `Hoje 1` para `Hoje 0 / Futuras 1`, sem erro. As revisões seguintes foram corretamente agendadas para o futuro e não podem ser falsificadas no mesmo instante; falta somente repetir essa sequência em quatro datas controladas no projeto Supabase de Teste.
