...
- [x] Qualidade do corte: 818 testes, lint e build concluídos sem falha em 2026-09-01; a suíte registra apenas o aviso conhecido de `Window.scrollTo` não implementado pelo ambiente jsdom.
...
- [ ] **Canal de suporte real.** O canal definido é `vourevisar@gmail.com`; CTAs de login, feedback, central do aluno e contato já usam e-mail. Em 2026-09-01, a página `/contato` em produção exibiu o `mailto:` correto, sem WhatsApp e sem erros de console, fora da sessão e dentro de uma conta de aluno comum. Falta confirmar o link publicado dentro de uma conta bloqueada. **Responsável: fundador** mantém a caixa postal; não é necessário enviar uma solicitação real nesta validação.
