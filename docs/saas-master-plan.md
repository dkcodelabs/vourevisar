...
- [ ] **Conferir autenticação em Safari real.** Abrir login, confirmação de e-mail e retorno de checkout em Safari antes de uma campanha maior. É uma verificação de compatibilidade externa; não há alteração pendente de código. **Responsável: fundador**, quando tiver uma sessão Safari disponível.
