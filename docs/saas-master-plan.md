...
- [ ] **Canal de suporte real.** O canal definido é `vourevisar@gmail.com`; CTAs de login, feedback, central do aluno e contato já usam e-mail. Em 2026-09-01, a página `/contato` em produção exibiu o `mailto:` correto, sem WhatsApp e sem erros de console. Falta confirmar o link publicado dentro de uma conta bloqueada e de uma conta comum. **Responsável: fundador** mantém a caixa postal; não é necessário enviar uma solicitação real nesta validação.
...
- [ ] Reduzir o baseline arquitetural em recortes: 35 acessos diretos ao Supabase em UI, 3 em utilitários e 9 páginas acima de 700 linhas (medição de 2026-09-01). Ordem inicial: `Editais.tsx` (3896 linhas), `Revisoes.tsx` (977) e áreas administrativas. Cada recorte deve mover persistência para service/hook, sem refatoração massiva.
...
- [x] Unificar a identidade de marca no runtime: a logo antiga, a animação de entrada, o fallback de rota e o loader foram substituídos pelo mesmo símbolo vetorial responsivo aos temas claro/escuro. O kit inclui SVG/PNG, versões transparentes do símbolo com `R` claro/escuro, favicon e imagem social 1200×630; o símbolo pode ser usado sem o nome e o wordmark não colore mais “Revisar” de azul. `prefers-reduced-motion` elimina o movimento não essencial. Em 2026-09-01, a marca foi validada renderizada em 1366×768 nos dois temas e em 375×812, sem erros no console; 819 testes, lint, typecheck e build passaram.
