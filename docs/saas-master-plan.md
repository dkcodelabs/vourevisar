...
- [ ] **Validação autenticada de acesso.** Exercitar em produção os perfis aluno pago, trial, sem acesso e admin, incluindo retorno do Customer Portal e confirmação de e-mail em desktop e Safari/mobile. É validação de experiência e sessão, não alteração de regras de acesso.
