...
- [ ] Reduzir o baseline arquitetural em recortes: 35 acessos diretos ao Supabase em UI, 3 em utilitários e 9 páginas acima de 700 linhas (medição de 2026-09-01). O primeiro recorte isolou o carregamento de Editais em `editaisPageService.ts`, com teste de escopo por usuário e propagação de erro, e removeu o hook manual sem consumidores de `useTopicReviewHistory`; a medição global ainda permanece em 35 porque outras consultas diretas continuam na página. Ordem seguinte: `Revisoes.tsx` (977) e áreas administrativas.
