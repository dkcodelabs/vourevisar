...
- [ ] Antes do refinamento visual, criar uma matriz/classificador de estados reais de entrada do produto para evitar vazio generico e dado inventado: sem edital cadastrado, edital cadastrado sem materias/topicos, edital com conteudo mas fora do ciclo, ciclo carregado sem conteudo valido, ciclo carregado com busca sem resultado, erro de carregamento/reconexao, primeiro acesso, trial ativo, assinatura vencida/sem acesso e assinatura paga/admin. Parcial em 2026-07-21: `getStudyEmptyStateKind` passou a separar primeiro acesso, edital vazio e edital pronto fora do ciclo em Dashboard, Revisoes, Estatisticas e Cadernos; ainda falta validar perfis de acesso e estados de carregamento/reconexao em navegador.
...
- [ ] Notificar o aluno quando a administracao liberar uma cota bonus de IA: a operacao agora grava `user_notifications` na mesma transacao do reset, com origem `admin_ai_quota_reset`, estado nao lido, mensagem de credito de cortesia e link para `Meus Editais`. Validado no banco remoto em 2026-07-21; falta confirmar visualmente a notificacao na sessao real do aluno e validar leitura/estado nao lido no navegador.
...
- [ ] Unificar hooks de assinatura/acesso para remover regras duplicadas de admin, owner, trial e assinatura paga. Parcial em 2026-07-21: além de `useSubscription`, `useSubscriptionInfo`, administração e estatísticas, o fallback de limite de IA em `ImportEditalModal` e o badge de `Matrizes IA` passaram a usar `getSubscriptionEntitlement`, incluindo `next_billing_date`; ainda falta consolidar completamente as fontes de cobrança, roles e leitura de conta em um único contrato.
- [x] Proteger ações administrativas de assinatura vinculada ao Asaas. Concluído em 2026-07-16: usuários com `asaas_subscription_id` não podem mais receber alteração manual pela UI, e `admin-rpc` bloqueia a mutação mesmo se chamada diretamente; usuários sem vínculo recebem apenas ações explicitamente manuais com confirmação. Edge Function publicada e validada pelo usuário no ambiente remoto.
- [x] Corrigir a divergência entre o acesso do aluno e o painel de assinaturas: em 2026-08-03, `/admin/subscription` passou a consumir a Edge Function autenticada `admin-billing`, que calcula acesso exclusivamente por `billing_subscriptions` e `billing_access_grants`, espelhando a regra de `get_stripe_billing_overview()`. `user_subscriptions`/Asaas entram somente como marcador histórico e nunca concedem acesso. As concessões administrativas agora gravam apenas `billing_access_grants`, recusam competir com assinatura Stripe vigente e não alteram cobranças.
- [ ] Remover os demais consumidores legados de `user_subscriptions` e Asaas fora da tela administrativa de assinaturas, começando pelas ações administrativas de ativar/desativar usuário e pelos hooks históricos. Eles não devem decidir nem alterar o entitlement Stripe; qualquer bloqueio global de conta precisa de estado canônico próprio e validação no guard de acesso.
