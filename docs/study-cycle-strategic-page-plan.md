...
- [ ] Zerar os 7 alertas restantes `authenticated_security_definer_function_executable` sem revogacao cega. Restam apenas helpers de RLS/role em `public`: `has_role` (2 assinaturas), `has_role_or_higher` (2 assinaturas), `is_admin`, `is_owner` e `is_user_active`. Recorte correto: mover para schema privado, trocar por policies sem helper `SECURITY DEFINER` exposto ou criar fronteira server-side especifica, validando admin, assinatura e RLS.
