...
- [x] Simplificar o resumo de topicos da materia para mostrar apenas um estado principal por vez, evitando informacoes juntas como `3 topicos` + `1/3 iniciados`. Prioridade de exibicao: se todos os topicos estiverem realmente concluidos, mostrar `3/3 topicos concluidos`; se a materia estiver fechada no ciclo, mostrar `3/10 topicos neste ciclo` quando esse dado for o mais util; caso normal, mostrar `1/3 topicos iniciados` ou `0/3 topicos iniciados`. Nao mostrar progresso geral, progresso do ciclo e conclusao real ao mesmo tempo; detalhes extras ficam em tooltip/painel. Implementado na Ciclo; removido tambem o chip redundante `Concluida no ciclo` da linha de metadados quando o resumo ja mostra o estado principal.
