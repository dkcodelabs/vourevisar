...
- [x] Publicar o frontend de homologação Stripe no domínio oficial mantendo a Stripe em sandbox. Concluído em 2026-07-31: variável publicável configurada na Vercel, CSP limitada aos endpoints oficiais necessários ao Stripe.js/Payment Element, build remoto promovido para `www.vourevisar.com.br` e `/conta/assinatura` validada em produção. Um build pré-compilado local com placeholders `[SENSITIVE]` foi detectado pela checagem visual e substituído por build remoto na Vercel. Enquanto as variáveis cliente estiverem marcadas como Sensitive, publicar este projeto com `vercel deploy --prod`, sem `vercel build --prod` local seguido de `--prebuilt`.
- [ ] Validar no modo de teste: renovação, `past_due`, cancelamento no fim do período, estorno vigente, estorno antigo e disputa; confirmar de forma isolada que retorno do checkout sem webhook não libera acesso. O cartão aprovado e a liberação exclusivamente após webhook já foram validados na homologação ponta a ponta abaixo. Em 2026-08-01, a recusa genérica oficial da Stripe (`card_declined`) também foi validada: o Payment Element permaneceu no checkout, exibiu `Seu cartão foi recusado`, a confirmação respondeu 402 como esperado e não houve navegação nem liberação de acesso. Na mesma data, o fluxo 3DS obrigatório foi validado: o desafio de autenticação da Stripe abriu no próprio checkout, foi concluído e a aplicação exibiu o estado de sucesso somente depois que a visão local confirmou a assinatura ativa. Em 2026-08-04, o estorno integral do ciclo vigente foi reenviado após a correção da resolução de fatura via PaymentIntent: a entrega `charge.refunded` retornou HTTP 200 e a assinatura do ciclo correspondente terminou em `canceled`. No mesmo sandbox, o estorno de um ciclo antigo não derrubou o ciclo posterior pago, que permaneceu `active`.
...
- [ ] Fazer auditoria visual em desktop, tablet e mobile e cobrir os estados carregando, sem configuração, erro, trial, ativo, pagamento pendente, cancelamento programado e acesso suspenso.
- [ ] Tratar em entrega própria os alertas restantes do `npm audit`: React Router 6 exige migração major para a linha corrigida e o toolchain do ESLint ainda traz `minimatch`/`brace-expansion`; não usar `npm audit fix --force` durante a migração financeira. O PostCSS foi atualizado para `8.5.25`.
...
- [x] Publicar e verificar as mudanças da Edge Function `stripe-webhook` no Supabase sandbox. Concluído em 2026-08-04: a função foi publicada após corrigir a resolução da fatura de estornos no formato atual da API Stripe; a entrega reenviada retornou HTTP 200. Não existe fila, token ou dependência operacional do Asaas neste domínio.
- [ ] Validar em sandbox a matriz: Pix pago, cartao aprovado, cartao recusado, cancelamento, vencimento, estorno do pagamento vigente, estorno antigo com pagamento posterior e concessao administrativa. Cartão aprovado/recusado, cancelamento, vencimento e os dois cenários de estorno foram comprovados até 2026-08-04; Pix e a validação comportamental de disputa continuam pendentes.
...
- [x] Publicar no Vercel o frontend que consome exclusivamente o contrato Stripe e validar `/planos`, `/checkout`, `/conta/assinatura`, administração e guard em produção. Concluído em 2026-08-04: a publicação foi isolada em um checkout limpo para não incluir alterações paralelas; a autoria Git foi ajustada para o e-mail `212580831+dkcodelabs@users.noreply.github.com`, reconhecido pela conta do projeto, e o commit `ed323b986efd9c6af67ccb742d79f3828da5750d` foi enviado para `main`. O deploy `dpl_F3WWXmVsKd8oLsuexfreua97rHtG` ficou `Ready`, foi promovido para `www.vourevisar.com.br` e as rotas públicas `/planos` e `/checkout?plan=monthly` responderam HTTP 200. A sessão autenticada validou em produção `/conta/assinatura` com o contrato Stripe, copy sem termos internos e estado de acesso sem dados legados. O guard e a administração foram validados por build, typecheck e 22 testes focados antes do push; a homologação financeira completa permanece registrada no item anterior.
- [ ] Virar o Stripe de sandbox para produção somente em entrega própria e após autorização explícita: criar produtos/preços live, configurar secrets e webhook live no Supabase, trocar a chave publicável na Vercel, repetir uma compra real controlada e confirmar os eventos no ledger. Decisão em 2026-08-04: permanecer em modo de teste enquanto a matriz financeira e a auditoria visual forem concluídas; não misturar essa virada com correções de produto.
