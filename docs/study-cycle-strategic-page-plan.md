...
- [ ] Publicar no Vercel o frontend que consome exclusivamente o contrato Stripe e validar `/planos`, `/checkout`, `/conta/assinatura`, administração e guard em produção. Bloqueio em 2026-08-04: os deployments `dpl_2pxoWZjRDyKfiwb4Sq5ownGihwSw` e `dpl_EWoZCvr5j3HsWAKEyot2e9uNar6S` foram recusados antes do build porque o autor do commit do repositório privado não está reconhecido como dono/membro do projeto Vercel. Não publicar um worktree com alterações paralelas enquanto a autoria Git e a origem do deploy não estiverem alinhadas.
