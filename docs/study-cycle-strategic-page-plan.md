...
- [x] Simplificar a leitura da assinatura com renovação cancelada: manter um único resumo `Renovação cancelada` com `Acesso até`, remover o alerta duplicado e mover o cartão atual para o histórico financeiro como `Cartão da assinatura`. O estado de teste gratuito, assinatura mensal/anual, pagamento pendente, reembolso e renovação cancelada permanece mutuamente exclusivo na visão canônica.
- [ ] Validar no domínio oficial a abertura do Customer Portal após a correção: para uma assinatura Live com renovação cancelada, o botão deve abrir a Stripe sem erro, permitir consultar faturas/cartão e oferecer apenas a reativação da renovação quando aplicável. Não cancelar, reembolsar ou alterar o plano durante essa validação.
