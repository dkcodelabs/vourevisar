...
- [ ] Planejar Pix como entrega financeira independente após o lançamento de cartão: confirmar elegibilidade da conta Stripe Brasil, criar preços avulsos separados para acesso mensal/anual sem renovação automática, definir a regra de expiração e homologar pagamento, expiração, estorno e reconciliação por webhook. Não reutilizar a assinatura de cartão nem o Customer Portal como se Pix fosse recorrente.
- [ ] Antes de habilitar produção Stripe, configurar chaves, preços e webhook de produção separados do sandbox, publicar por um único caminho reprodutível e repetir a matriz crítica com uma conta de produção controlada. Parcial em 2026-08-05: as seis Edge Functions Stripe publicadas foram baixadas para comparação isolada; catálogo, checkout, portal, histórico e admin já coincidiam, e o webhook local foi atualizado com a mesma resolução de cobrança via PaymentIntent presente no remoto. O lançamento simples e seguro não exige um segundo projeto Supabase: Stripe Test mode e Live mode já mantêm chaves, clientes, preços, pagamentos e webhooks separados. No corte, criar os dois preços e o webhook em Live mode, trocar uma única vez os quatro secrets canônicos do projeto (`STRIPE_SECRET_KEY`, preços mensal/anual e `STRIPE_WEBHOOK_SECRET`) e a chave publicável da Vercel de `pk_test` para `pk_live`, então repetir a matriz com uma conta real controlada. O sandbox permanece no Dashboard Stripe para testes anteriores ao corte; um segundo Supabase/Vercel só será avaliado depois do lançamento se houver necessidade real de staging contínuo, pois o schema canônico mantém um único cliente Stripe por usuário. O bloqueio operacional continua sendo normalizar a publicação Vercel/Git e não tratar o ambiente local como evidência de que produção está atualizada.
