...
- [ ] Zerar os alertas restantes `authenticated_security_definer_function_executable` sem revogacao cega. A proxima leva ja esta preparada no codigo com Edge Function `user-rpc`, dispatcher privado `user_rpc_dispatch` e migration `20260707125900_move_user_security_definer_rpcs_to_edge.sql`, movendo ciclo/merge/log/assinatura do usuario para uma fronteira Edge com contexto de usuario preservado. A migration deve ser aplicada somente depois do frontend novo estar em producao, para nao quebrar a versao antiga que ainda chama RPC direto. Depois dela, devem restar principalmente helpers de RLS (`has_role*`, `is_admin`, `is_owner`, `is_user_active`), cujo recorte correto e mover para schema privado ou reescrever policies sem helper `SECURITY DEFINER`, validando admin, assinatura e RLS.
