...
- [ ] Tratar em entrega própria os alertas restantes do `npm audit`: React Router 6 exige migração major para a linha corrigida; não usar `npm audit fix --force` para esconder essa mudança. O PostCSS foi atualizado para `8.5.25`. Em 2026-08-06, o Quality Gate revelou o alerta alto `GHSA-hq66-cqwq-w95j` no `pdfjs-dist`; a dependência foi fixada em `6.2.108`, o extrator migrou para o build `legacy` compatível e para o cleanup do `PDFDocumentLoadingTask`, e um teste com PDF real foi adicionado. Os alertas altos transitivos de desenvolvimento em `js-yaml` e `undici` também foram eliminados pelo lockfile. `npm audit --omit=dev --audit-level=high`, teste focado, typecheck e lint passaram localmente; falta confirmar o Quality Gate remoto. Permanecem somente os dois alertas moderados do React Router, que exigem a migração major planejada.
