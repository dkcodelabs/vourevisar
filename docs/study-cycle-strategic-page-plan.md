...
- [x] Definir a apresentacao quando o ciclo tiver mais de um edital ativo; a primeira versao usa o primeiro edital retornado pelo ciclo como contexto principal e exibe badge contextual de editais combinados no painel estrategico.
- [x] Definir semanticamente a metrica `ciclos anteriores` antes de exibir em `Meus Editais > Carregar Edital`: a contagem de rodadas/giros concluídos pertence exclusivamente ao ciclo de estudos global (`user_cycles.ciclos_realizados`). Em `Meus Editais`, os cards exibem métricas reais do edital (`total de matérias`, `tópicos`, `horas estudadas` e `dias até a prova`), eliminando contagens ambíguas que poderiam induzir o aluno a erro.
- [x] Levar os cards antigos que o usuario deseja preservar para `/reveal-cards` antes de remover definitivamente componentes legados do repositorio. Concluido com `src/pages/RevealCards.tsx` e rota `/reveal-cards` no `App.tsx`.
...
- [x] Redesenhar card de conclusao de rodada do ciclo (`CycleFirstContactFinishedPanel`) e fluxo de transicao: eliminar o rotulo enganoso `Primeiro contato encerrado` quando ha topicos virgens no edital; diferenciar claramente `Rodada concluida` (todas as materias da fila cumpridas no ciclo) de `Primeiro contato do edital finalizado` (100% dos topicos iniciados); remover a metrica ambigua `Sem topico novo: 0/6` e caixas quebradas lado a lado; apresentar metricas limpas de rodada (`Materias na rodada: X de X cumpridas`, `Avanco no edital`, `Tempo no ciclo`, `Revisoes ativas`) com destaque unificado de esforco; e recolher a lista redundante de materias concluidas abaixo do card sob accordion discreto para visual limpo e foco no proximo ciclo. Testes, lint e build aprovados.
...
- [x] Promover padroes locais da Ciclo para componentes/tokens globais quando a tela for aprovada: classes e tokens consolidados em `index.css` (`app-glass`, `app-gradient-panel`, `app-strategic-map-panel`, `app-map-chip`, `app-control`, `app-field`, `app-hairline`), componente de estado `PremiumStateCard.tsx` e sistema de notificações `PremiumToastViewport.tsx`.
...
- [x] Substituir o padrao atual de toast/snackbar de acoes, incluindo a mensagem verde como `Ordem do ciclo atualizada!`, por um componente mais premium. Concluido com `PremiumToastViewport.tsx` e `src/lib/toast.ts`: viewport fixo acessivel, roles semanticas, pause/resume no hover/focus, glow refinado, cores semanticas e acao opcional de desfazer/callback.
...
- [x] Consolidar a sincronizacao do nome do ciclo dentro da RPC `atomic_archive_edital_from_cycle` em uma etapa segura de banco: migration `20260825200500_sync_cycle_name_on_archive.sql` recalcula e persiste atomicamente `user_cycles.name` a partir dos editais remanescentes diretamente no PostgreSQL, mantendo o fallback no frontend e cobertura em `cycleArchivePersistence.test.ts` e `cycleUnloadService.test.ts`. Concluído em 2026-08-25.
...
- [x] Reavaliar exibicao de `menos cobrado` apenas quando o mapa de cobranca do edital estiver suficientemente completo; com analise parcial, esse dado pode enganar. Concluido: o painel estrategico expoe exclusivamente sinais consolidados de `Maior cobranca por materia`, `Topico de maior cobranca` e `Maior peso pendente`, evitando conclusoes falsas de baixa cobranca por falta de dados.
...
- [x] Se houver peso por bloco, nao distribuir automaticamente entre disciplinas: o motor de extracao (`extract-edital/index.ts`), os perfis de banca (`_shared/bank-profiles/`) e o frontend (`AiReviewStep.tsx`) tratam `block_only` de forma conservadora, preservando o peso das matérias como nulo e exibindo aviso explicativo para o aluno sem injetar pesos artificiais. Concluído em 2026-08-25.
...
- [x] Avisar quando houver topicos com maior volume/incidencia iniciados, mas ainda nao revisados, se houver dado de revisao confiavel. Concluido em `studyCycleAlerts.ts`: detecta 1º contato ha mais de 48h com 0 revisoes em topicos de alta cobranca ou materias com peso, com CTA direto `Ver revisoes` que navega para `/revisoes`.
...
- [x] Avisar sobre incidencia ausente apenas se houver caminho claro para rodar/gerar analise da IA. Concluido: `getTopicStrategicIncidence` e `studyCycleAlerts` integraram suporte nativo a `topics.incidence_level` ('high' | 'medium' | 'low') com fallback conservador por volume, sem exibir alertas de erro vazios para o aluno.
...
- [x] Topico importante nao revisado: implementado com base em 1º contato ha mais de 48h sem revisao e presenca de cobranca/peso importante, sem acoplamento indevido com FSRS.
...
- [x] Manter uma única ação financeira durante a janela de arrependimento: em 2026-08-24, o Portal Stripe deixou de ser exibido enquanto o pedido de cancelamento e reembolso integral estiver disponível. Assim, não há dois caminhos de cancelamento com efeitos distintos: o único CTA é `Cancelar compra e pedir reembolso`; cobranças e reembolsos ficam no Histórico financeiro do produto. Após o prazo, `Gerenciar assinatura` abre o Portal completo da Stripe para faturas, cartão, cancelamento no fim do período e reativação.
- [x] Tratar o bloqueio de segunda assinatura como estado orientado, não como falha de checkout: em 2026-08-24, o retorno `subscription_already_active` passou a mostrar `Seu plano já está ativo` com CTA para `Minha assinatura`, sem oferecer repetição inútil da tentativa. A Edge Function continua recusando HTTP 409 para proteger contra cobrança duplicada.
...
## Backlog de Inovação: Cadernos & IA (`/cadernos`)

- [ ] **IA nos Cadernos (Anotações Inteligentes)**:
  - Gerar resumos rápidos e esquematizados a partir das anotações pessoais do aluno por tópico ou matéria.
  - Gerar questões de fixação e simulados rápidos baseados no conteúdo e anotações registradas.
  - Extração de pontos cegos / tópicos de maior dúvida a partir do texto das anotações.

## Backlog de Projeção Estratégica: Carga Total de Estudos até a Prova

- [x] **Projeção da Carga Total Combinada de Revisões**:
  - Implementado cálculo exato da carga total de trabalho até a data da prova: cada tópico novo projeta 4 revisões de consolidação (R1 a R4) e cada tópico em andamento projeta `4 - review_count` revisões restantes.
  - Exibidas no card de ritmo (`RevisoesHeader.tsx`) as 3 métricas diárias: **Tópicos Novos** (`novos/dia`), **Revisões do Programa** (`revisões/dia`) e **Meta Total** (`~estudos/dia`).
  - Blindados os edge cases de prova hoje/passada (`state: 'exam_date_past'`), precisão numérica sem arredondamento precoce e 100% de cobertura de testes unitários e de UI em `studyCycleMetrics.test.ts` e `RevisoesHeader.test.tsx`. Concluído em 2026-08-25.

## Backlog de Inteligência e Extração de Editais (`/editais` & IA)

- [x] **Refatoração Anti-Monólito do Modal de Importação (`ImportEditalModal.tsx`)**:
  - Monólito de 3.071 linhas fatiado em hooks dedicados (`useCatalogEditais.ts`, `useAiEditalExtraction.ts`) e subcomponentes visuais de responsabilidade única (`AiInputStep.tsx`, `AiCargoSelectionStep.tsx`, `AiProgressStep.tsx`, `EditalSuggestionDrawer.tsx`).
  - Código do orquestrador reduzido drasticamente, mantendo 100% das regras de negócio, Edge Functions, prompts e integridade de tipos.
  - Validado com `npm run lint` (0 erros), `npm run test:run` (185 arquivos / 723 testes aprovados) e `npm run build` bem-sucedido. Concluído em 2026-08-25.
- [x] **Expansão dos Perfis Declarativos de Bancas (`_shared/bank-profiles/`)**:
  - Criados perfis dedicados em JSON para as 5 novas bancas de grande volume: **Vunesp** (`vunesp.json`), **Instituto AOCP** (`instituto_aocp.json`), **Quadrix** (`quadrix.json`), **IBFC** (`ibfc.json`) e **IADES** (`iades.json`), dobrando a cobertura nativa de bancas do sistema para 10 bancas.
  - Registrados no `extract-edital/index.ts` com aliases oficiais, marcadores estruturais e instruções adaptativas sem código if/else pontual.
  - Validado com 100% de aprovação na suíte de 185 arquivos de testes (724 testes). Concluído em 2026-08-25.
- [ ] **Validação com Editais Reais em Produção**:
  - Testar importação de PDFs reais de editais das 5 novas bancas (VUNESP, Instituto AOCP, Quadrix, IBFC e IADES) para confirmar extração 100% fiel de cargos, disciplinas e pesos na interface.

## Página Evolução baseada em dados reais (`/estatisticas`)

- [x] Substituir a tela legada por uma página de evolução orientada a decisão, sem abas mortas, pontos artificiais, estimativas por dificuldade ou controles de migração expostos ao aluno.
- [x] Restringir o MVP ao ciclo ativo e respeitar matérias/tópicos ativos, merges e tópicos ocultos já tratados pelos contratos canônicos do produto.
- [x] Usar `study_sessions.session_duration_minutes` como fonte canônica única do tempo. `topic_review_history.study_duration_minutes` não é somado porque pode representar o mesmo contato e causaria dupla contagem; `pomodoro_sessions` permanece fora desta tela.
- [x] Exibir progresso real como cobertura (`tópicos iniciados / total`) e consolidação (`completed / total`), sem transformar conclusão de ciclo/giro em domínio do conteúdo.
- [x] Reutilizar a classificação do motor adaptativo atual para `Aprendendo`, `Fixando` e `Dominando`. Não chamar a heurística existente de FSRS e não publicar percentual de retenção inventado.
- [x] Manter revisões vencidas, de hoje e futuras como dimensão separada da maturidade de memória.
- [x] Implementar períodos de 7, 14 e 30 dias com comparação apenas quando existe base real no período anterior; não fabricar variação quando o denominador anterior é zero.
- [x] Implementar visão por matéria com tempo registrado, cobertura, consolidação, revisões vencidas e peso do edital quando esse dado existe.
- [x] Gerar um único foco estratégico determinístico e explicável a partir dos dados do ciclo, com ação para Revisões ou Ciclo de estudos.
- [x] Manter `src/pages/Statistics.tsx` como orquestradora fina, com TanStack Query em `useCycleStatistics`, persistência/leitura em service, cálculos puros em util e blocos visuais isolados em `src/components/statistics/`.
- [x] Cobrir as fórmulas puras, deduplicação de sessões, ausência de comparação falsa, troca de período, hierarquia da página e navegação do foco com Vitest/Testing Library.
- [x] Validar visualmente a superfície completa em desktop, tablet e mobile. Em 2026-08-25, uma fixture local temporária — removida após a validação e nunca persistida — exercitou os componentes reais em 390x844, 768x1024 e 1440x900, dark/light e `Todo o ciclo`, sem overflow horizontal, overlay do Vite ou erro de console. A inspeção identificou e corrigiu a margem negativa que comprimia os rótulos do eixo do gráfico.
- [x] Reconciliar o histórico antes de oferecer `Todo o ciclo` e melhor sequência histórica. Auditoria somente leitura no Supabase em 2026-08-25 confirmou 16 sessões entre 2026-07-07 e 2026-08-25, 3 usuários, zero duração não positiva, zero sessão sem ciclo/matéria e zero duplicata semântica. A opção agora pagina todas as `study_sessions` do usuário/ciclo, calcula a janela desde `data_inicio_ciclo`, não cria comparação artificial e mostra a melhor sequência real do ciclo.
- [x] Auditar e remover os consumidores legados de estatísticas. Em 2026-08-25, busca completa em `src/` provou que `useRealStatistics.tsx`, `useAdvancedStatistics.tsx`, `RealDataIndicator` e os oito blocos/barrel antigos não tinham consumidores além da antiga página já substituída; os arquivos foram removidos. A entrega final passou em lint, typecheck, 717 testes e build.
- [x] Corrigir o falso estado vazio após carregar um edital no ciclo. Em 2026-08-25, a mesma sessão autenticada confirmou um ciclo ativo com 6 matérias e 103 tópicos; a divergência vinha do cache recente de `null` da query da Evolução. `useCycleStatistics` agora sempre reconcilia os dados ao montar a página, com teste de regressão específico para o retorno à rota antes do fim do `staleTime`.
