...
- [ ] Criar metrica unificada de ritmo ate a prova para exibir na pagina Ciclo, na pagina Revisoes e no Painel: abaixo do titulo do painel/area estrategica, mostrar quantos dias faltam para a prova, quantos dias restam para iniciar os topicos novos ate a prova e quantos topicos novos por dia seriam necessarios. O calculo deve usar dados reais disponiveis: topicos ativos ainda nao iniciados, data da prova, historico real de inicio/primeiro contato, tempo medio real para iniciar um novo topico quando existir e ritmo recente do aluno. Se nao houver dados suficientes, mostrar estado honesto e nao inventar previsao.
  ...
  - [ ] Corrigir o escopo canonico das metricas contextuais para impedir vazamento de historico global quando o usuario troca/carrega edital no ciclo: Painel (`Sua trajetoria`, `Consistencia recente`), Revisoes (`Tendencia de estudos`, `Engajamento semanal`) e qualquer resumo de ritmo/contexto devem filtrar apenas topicos ativos do `user_cycles.ciclo_atual`. Implementacao aplicada em Dashboard, Revisoes, `useCycleStatsData` e `useRealStatistics`; topicos sem `difficulty_level` nao entram mais automaticamente como `medio`. Spec em `docs/superpowers/specs/2026-06-22-cycle-analytics-scope-design.md`. Pendente validacao visual autenticada dessas superficies.
...
- [ ] Investigar em recorte separado o warning nao-bloqueante vindo de Revisoes/merge: `map.unifiedSubjects is not iterable` em `cycleMergeService.ts` durante `markTopicAsReviewed`. O aviso apareceu no console durante QA da Ciclo, mas aponta para propagacao de historico/progresso unificado e nao deve ser corrigido misturado ao polimento visual.
