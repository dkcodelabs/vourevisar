-- =====================================================
-- REMOVER E RECRIAR FUNÇÕES DE ASSINATURA - VERSÃO CORRIGIDA
-- =====================================================

-- Primeiro, remover todas as funções existentes que podem estar conflitando
DROP FUNCTION IF EXISTS get_subscription_info(UUID);
DROP FUNCTION IF EXISTS has_active_subscription(UUID);
DROP FUNCTION IF EXISTS start_paid_subscription(UUID, subscription_plan, INTEGER);
DROP FUNCTION IF EXISTS cancel_subscription(UUID, BOOLEAN);
DROP FUNCTION IF EXISTS update_expired_subscriptions();
DROP FUNCTION IF EXISTS update_user_subscription(UUID, subscription_plan, subscription_status, TIMESTAMP WITH TIME ZONE, TIMESTAMP WITH TIME ZONE, TIMESTAMP WITH TIME ZONE, TIMESTAMP WITH TIME ZONE);
...
-- Agora recriar todas as funções com as assinaturas corretas
...
-- Função para obter informações da assinatura (retorna JSON)
...
-- Função para ativar assinatura paga
