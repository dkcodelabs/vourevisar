...
export const StudyCycleSubjectCard: React.FC<StudyCycleSubjectCardProps> = ({
  ...
  if (isFullyCompleted && viewMode === 'grid') {
    return (
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-md overflow-hidden flex flex-col">
        <div className="p-6">
          <h3 className="text-base text-zinc-800 dark:text-zinc-200" style={{ fontWeight: 700 }}>{subject.name}</h3>
        ...
        <div className="p-4 bg-gray-50 dark:bg-slate-800/50 flex-grow">
          <h4 className="text-xs font-medium text-gray-700 dark:text-slate-300 mb-3 px-2">Tópicos concluídos:</h4>
              ...
              {isActionable && (
                <div className="mt-4 pt-4 border-t border-gray-200 dark:border-slate-700 flex justify-end">
                  <button
                    ...
                    className="bg-sky-600 text-white font-bold py-2 px-4 rounded-lg transition-all duration-300 hover:bg-sky-700 disabled:bg-gray-300 dark:disabled:bg-slate-600 disabled:cursor-not-allowed disabled:text-gray-500 dark:disabled:text-slate-500 text-sm"
                  >
  ...
  return (
    <div className={`${cardBaseClasses} flex flex-col ${focusClasses}`}>
      ...
      {isActionable && (
        <div className="p-4 bg-white dark:bg-slate-800 border-t border-gray-200 dark:border-slate-700 mt-auto flex justify-end">
          <button
            onClick={handleComplete}
            disabled={markedTopicIds.size === 0}
            className="bg-sky-600 text-white font-bold py-2 px-4 rounded-lg transition-all duration-300 hover:bg-sky-700 disabled:bg-gray-300 dark:disabled:bg-slate-600 disabled:cursor-not-allowed disabled:text-gray-500 dark:disabled:text-slate-500 text-sm"
