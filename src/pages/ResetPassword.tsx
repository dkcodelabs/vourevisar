...
const ResetPassword = () => {
    ...
    const handleAuthRedirect = async () => {
      ...
      try {
        ...
        const acceptRecoverySession = async (session: Session) => {
          ...
          if (!authMethods.hasPassword) {
            ...
            toastManager.warning(
              "Esta conta usa o acesso pelo Google e não possui senha no vouRevisar. Continue com Google.",
            );
        ...
        if (code) {
          ...
          try {
            ...
            if (authError.name === "AuthPKCECodeVerifierMissingError") {
              console.warn(
                "PKCE Verifier Missing: Browser executing the link is different from the one that requested it.",
              );
              toastGate.notifyError(
                "Por segurança, abra o link no mesmo navegador/dispositivo que solicitou.",
                "AUTH-PKCE-MIS",
                { severity: "medium" },
              );
            } else {
        ...
        errorService.report(error, {
          module: "auth",
          action: "redirect_handler",
          userMessage: "Erro ao processar link de recuperação",
        });
        navigate("/login");
      } finally {
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    if (password !== confirmPassword) {
      toastGate.notifyError("As senhas não coincidem", "AUTH-PASS-MISMATCH", {
        severity: "low",
      });
      return;
    }
    ...
    try {
      ...
      if (!sessionData.session) {
        ...
        toastGate.notifyError(
          "Sessão expirada. Tente novamente com um novo link",
          "AUTH-SESS-EXP",
          { severity: "medium" },
        );
        navigate("/login");
      ...
      <motion.div initial={false} className="w-full">
        ...
        <form onSubmit={handleResetPassword} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl bg-[#1765dc] py-3.5 font-bold text-white shadow-lg shadow-blue-600/10 transition-colors hover:bg-[#1254bd] disabled:opacity-70 sm:py-4"
          >
