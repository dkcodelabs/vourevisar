...
const ConfirmEmail = () => {
  ...
  const hasConfirmationError =
    confirmationStatus === "expired" || confirmationStatus === "error";
  ...
  useEffect(() => {
    ...
    const confirmedEmail = localStorage
      .getItem("confirmedEmail")
      ?.trim()
      .toLowerCase();
    if (
      resolvedEmail &&
      confirmedEmail === resolvedEmail.trim().toLowerCase()
    ) {
    ...
    const cooldownUntil = Number(
      localStorage.getItem("pendingConfirmationCooldownUntil") || 0,
    );
  ...
  useEffect(() => {
    const handleConfirmationFromAnotherTab = (event: StorageEvent) => {
      const confirmedEmail =
        event.key === "confirmedEmail"
          ? event.newValue?.trim().toLowerCase()
          : null;
      if (
        !confirmedEmail ||
        !email ||
    ...
    return () =>
      window.removeEventListener("storage", handleConfirmationFromAnotherTab);
  }, [email, navigate]);
    ...
    try {
      ...
      if (!userError && userData.user && hasConfirmedEmail(userData.user)) {
        ...
        setResendFeedback({
          tone: "error",
          message:
            "Este email já foi confirmado. Volte ao login e entre com sua senha.",
        });
        toastGate.notifyError(
          "Este email já foi confirmado. Entre pelo login.",
          "AUTH-ALREADY-CONFIRMED",
          { severity: "low" },
        );
        return;
      }
      ...
      if (error) {
        ...
        if (error.status === 429) {
          ...
          localStorage.setItem(
            "pendingConfirmationCooldownUntil",
            String(Date.now() + 60_000),
          );
          setResendFeedback({
            tone: "error",
            message:
              "O serviço de email está temporariamente limitando novos envios. Aguarde cerca de 1 minuto antes de tentar novamente.",
          });
          toastGate.notifyError(
            "O envio está temporariamente limitado. Aguarde um minuto.",
            "AUTH-RATE-LIMIT",
            { severity: "medium" },
          );
        } else if (
          ...
          setResendFeedback({
            tone: "error",
            message:
              "Não foi possível reenviar agora. Aguarde um momento e tente novamente.",
          });
          toastGate.notifyError(
      ...
      setResendFeedback({
        tone: "success",
        message:
          "Se a confirmação ainda estiver pendente, o email poderá chegar em alguns minutos. Verifique sua caixa de entrada, spam ou lixo eletrônico.",
      });
      ...
      toastGate.notifyError("Erro ao reenviar email.", "AUTH-RESEND-UNK", {
        severity: "medium",
      });
    } finally {
          ...
          <ol className="text-sm text-content-muted space-y-2 list-decimal list-inside">
            {hasConfirmationError || isAwaitingConfirmation ? (
              ...
              <>
                ...
                <li>
                  Se receber uma mensagem do vouRevisar, clique em "Confirmar
                  meu cadastro"
                </li>
