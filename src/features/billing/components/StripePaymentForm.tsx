...
export const StripePaymentForm = ({
  ...
  return (
    <form
      ...
      className="rounded-[1.75rem] border border-white/80 bg-white/85 p-4 shadow-[0_28px_80px_-42px_rgba(36,24,77,0.5)] backdrop-blur-xl sm:p-5"
    >
      <div className="mb-3 flex items-start justify-between gap-4 border-b border-[#ebe7f5] pb-3">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.18em] text-[#746c87]">Total de hoje</p>
          <p className="mt-0.5 text-3xl font-black tracking-[-0.04em] text-[#17122b]">{totalToday}</p>
          {appliedPromotion && (
      ...
      <div className="mb-3 rounded-2xl border border-[#ded8ed] bg-[#faf9ff] p-3">
        {appliedPromotion ? (
          <div className="flex items-center justify-between gap-3 rounded-xl bg-[#eaf8f4] px-3 py-2">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.14em] text-[#126f61]">Código aplicado</p>
              <p className="mt-0.5 text-sm font-black text-[#126f61]">{appliedPromotion.promotionCode}</p>
            </div>
            <button
              type="button"
          ...
          <details className="group">
            <summary className="flex min-h-9 cursor-pointer list-none items-center gap-2 text-sm font-black text-[#2f2940] marker:hidden">
              <Tag className="h-4 w-4 text-[#5b47eb]" />
              Tenho código de divulgação
              <span className="ml-auto text-xs font-semibold text-[#746c87] group-open:hidden">Opcional</span>
            </summary>
            <div className="mt-2 flex flex-col gap-2 sm:flex-row">
              <input
                ...
                autoComplete="off"
                className="min-h-10 flex-1 rounded-xl border border-[#d8d2e5] bg-white px-3 text-sm font-bold uppercase text-[#2f2940] outline-none transition placeholder:normal-case placeholder:font-medium placeholder:text-[#8b8498] focus:border-[#6b4df5] focus:ring-2 focus:ring-[#6b4df5]/15"
              />
              <button
                type="button"
                onClick={handleApplyPromotionCode}
                disabled={!promotionCode.trim() || !checkout || isUpdatingPromotion}
                className="min-h-10 rounded-xl bg-[#ece8ff] px-5 text-sm font-black text-[#5b47eb] transition hover:bg-[#e2dcff] disabled:cursor-not-allowed disabled:opacity-50"
              >
      ...
      {requireContractAcceptance && (
        <div className="mt-3 border-t border-[#ebe7f5] pt-3 text-sm text-[#433b56]">
          <p className="text-xs font-bold leading-5 text-[#686078]">
            Hoje: {totalToday}. Renovação {plan === 'annual' ? 'anual' : 'mensal'} pelo valor vigente. Próxima cobrança estimada em {getEstimatedRenewalDate(plan)}.
          </p>
          <details className="mt-2 text-xs font-semibold leading-5 text-[#686078]">
            <summary className="cursor-pointer font-black text-[#5b47eb]">Ver regras de renovação e reembolso</summary>
            <p className="mt-1.5">Você pode cancelar futuras renovações em Minha assinatura. Em contratações online elegíveis, é possível desistir em até 7 dias e solicitar reembolso integral. O teste gratuito anterior é separado e não gera cobrança automática.</p>
          </details>
          <label className="mt-3 flex cursor-pointer items-start gap-3 rounded-xl bg-[#f6f3ff] p-2.5 font-bold">
            <input
              type="checkbox"
      ...
      <div className="mt-3 grid gap-1.5 text-xs font-bold text-[#686078] sm:grid-cols-2">
