...
describe("PracticeGenerationDialog", () => {
  it("confirma o lote pronto e devolve o aluno ao compositor preservado", () => {
    const onReturnToComposer = vi.fn();
    render(<PracticeGenerationDialog open state="ready" topic={topic} onOpenChange={vi.fn()} onReturnToComposer={onReturnToComposer} onChooseAnotherTopic={vi.fn()} />);
  ...
  it("mantém a geração em andamento visível e permite decidir depois", () => {
    ...
    render(<PracticeGenerationDialog open state="preparing" topic={{ ...topic, hasReadyPackage: false, isGenerating: true, questionCount: 0, flashcardCount: 0 }} onOpenChange={onOpenChange} onReturnToComposer={vi.fn()} onChooseAnotherTopic={vi.fn()} />);

    expect(screen.getByRole("dialog", { name: "Preparando material" })).toBeInTheDocument();
    expect(screen.getByText(/lote ficará disponível neste treino/i)).toBeInTheDocument();
  ...
  it("explica a falha sem prometer material que não existe", () => {
    render(<PracticeGenerationDialog open state="failed" topic={{ ...topic, hasReadyPackage: false, questionCount: 0, flashcardCount: 0 }} onOpenChange={vi.fn()} onReturnToComposer={vi.fn()} onChooseAnotherTopic={vi.fn()} />);
