...
export const useNextSubjects = (subjects: Subject[], userCycle: UserCycle | null, userSettings: { subjects_per_day: number } | null) => {
  const { nextSubjects, subjectsByStatus } = useMemo(() => {
    ...
    // CORREÇÃO: Excluir matérias que estão em materias_pendentes (já foram para próximo ciclo)
    ...
    const subjectsNotInCycle = subjects
      .filter(subject => 
        ...
        !materiasPendentes.includes(subject.id) // NOVA CONDIÇÃO: excluir pendentes
      )
    ...
    console.log('🔄 useNextSubjects - Classificação por status:', {
      ...
      materiasPendentesExcluidas: materiasPendentes.length,
      materiasPendentesNomes: materiasPendentes.map(id => subjects.find(s => s.id === id)?.name || 'NOT_FOUND')
    });
