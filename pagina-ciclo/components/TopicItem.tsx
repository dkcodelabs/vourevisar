...
import { ReviewInterval } from '../types';
...
const REVIEW_STATUS_CONFIG: Record<ReviewInterval, { text: string; className: string }> = {
  [ReviewInterval.NOT_STARTED]: { text: 'Não iniciado', className: 'bg-gray-200 text-gray-700 dark:bg-slate-600 dark:text-slate-300' },
  ...
  [ReviewInterval.COMPLETED]: { text: 'Revisado', className: 'bg-emerald-200 text-emerald-800 dark:bg-emerald-800/50 dark:text-emerald-300' },
};

const TopicItem: React.FC<TopicItemProps> = ({ topic, isMarkedInSession, onToggleMark, onOpenNotes, isSubjectFinished, isActionable }) => {
  ...
  if (isSubjectFinished) {
    return (
      <div className="flex items-center justify-between w-full px-4 py-2">
        <span className="text-gray-700 dark:text-slate-400">{topic.name}</span>
  ...
  return (
    <div className={`${baseClasses} ${bgClasses}`}>
      <span className="text-gray-800 dark:text-slate-300">{topic.name}</span>
