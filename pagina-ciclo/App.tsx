...
import { SubjectStatus, ReviewInterval } from './types';
import { initialSubjects } from './data/initialData';
import { STATUS_CONFIG } from './constants';
import SubjectCard from './components/SubjectCard';
import { GridIcon, ListIcon, ChevronsDownIcon, ChevronsUpIcon, CheckCircleIcon } from './components/Icons';
import NotesModal from './components/NotesModal';
import CompletionMessage from './components/CompletionMessage';
import ThemeSwitcher from './components/ThemeSwitcher';

// --- Main App Component ---
const LOCAL_STORAGE_KEY = 'reviewAppSubjects_v4';
const LOCAL_storage_view_key = 'reviewAppViewMode';
...
const App: React.FC = () => {
  ...
  const handleStartNewCycle = useCallback(() => {
    setSubjects(currentSubjects => {
      ...
      if (activeSubjects.length === 0 && completedCycleSubjects.length > 0) {
        ...
        // The subjects just completed are already in COMPLETED_CYCLE status, so `activeSubjects` contains the next candidates.
        const newFocusIds = new Set(activeSubjects.slice(0, STUDY_FOCUS_COUNT).map(s => s.id));
        setStudyFocusSubjectIds(newFocusIds);
        return currentSubjects; // No change to subjects themselves
      }
    });
  }, []);
  ...
  const groupedSubjects = useMemo(() => {
    return subjects.reduce((acc, subject) => {
      ...
      return acc;
    }, {} as Record<SubjectStatus, Subject[]>);
  }, [subjects]);
  ...
  const isDayCompleted = useMemo(() => {
    if (studyFocusSubjectIds.size === 0) {
        // If there are no more subjects to focus on, the day/cycle is done.
  ...
  const handleSaveNotes = useCallback((subjectId: string, topicId: string, updatedData: Partial<Topic>) => {
    setSubjects(currentSubjects =>
  ...
  const renderSection = (status: SubjectStatus) => {
    ...
    const containerClasses = viewMode === 'grid' || isFinishedSection
    ...
    return (
      <section key={status} className="mb-12">
        <div className="flex items-center mb-6">
          ...
          <h2 className={`text-2xl font-bold text-gray-900 dark:text-slate-100 border-b-2 ${config.borderColor} pb-2`}>
            {config.title}
          </h2>
        </div>
        <div className={containerClasses}>
          {sectionSubjects.map((subject) => (
            <SubjectCard
              ...
              viewMode={isFinishedSection ? 'grid' : viewMode}
  ...
  return (
    <div className="min-h-screen text-gray-900 dark:text-slate-100 font-sans">
      <header className="bg-white/80 dark:bg-slate-800/60 shadow-md sticky top-0 z-40 backdrop-blur-lg">
        <div className="container mx-auto flex flex-col items-center justify-center text-center p-6 h-28">
          ...
          <p className="hidden md:block text-gray-600 dark:text-slate-400 mt-2">Mantenha seus estudos em dia com o ciclo de revisões.</p>
        </div>
      </header>
        ...
        {isDayCompleted ? (
          <section className="mb-12">
            <div className="flex items-center mb-6">
                ...
                <h2 className="text-2xl font-bold text-gray-900 dark:text-slate-100 border-b-2 border-emerald-500 pb-2">
      ...
      <footer className="text-center p-6 text-gray-500 dark:text-slate-400 text-sm">
